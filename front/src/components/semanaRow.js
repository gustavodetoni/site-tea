...
const SemanaRow = ({ data, atividade, sequenciaEscolhida, setSequenciaEscolhida, pacienteView, readOnly = false }) => {
      ...
      <td>
        <div>
          ...
          <div className="atividades-escolhidas">
            {atividadesDaSemana &&
              atividadesDaSemana[data.semana]?.map((item, index) => (
                  ...
                  {pacienteView ? (
                    <>
                      ...
                      <label
                        ...
                        key={index}
                        className={`atividade-paciente ${item.isCheck ? "check" : ""} ${readOnly ? "readonly" : ""}`}
                      ...
                      <Tooltip
                        target={`#label-${item.value}-${index}-${data.semana.slice(7)}`}
                        className="tooltip-atividade"
                      >
                    ...
                    <label
                      key={index}
                      onClick={() => !readOnly && removeAtividade(data.semana, index)}
...
function getHabilidadeSlug(activitySlug) {
  ...
  const habilidade = mapAtividades.find((item) => item.itemSlugs.includes(activitySlug))
